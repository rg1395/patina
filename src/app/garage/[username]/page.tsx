...
export default async function PublicGaragePage({ params }: { params: { username: string } }) {
  ...
  return (
    <div>
      {/* Header */}
      <div style={{ background: C.dark, padding: "clamp(1.5rem,5vw,3.5rem) clamp(1rem,4vw,2.5rem)" }}>
        <div style={{ maxWidth: "1280px", margin: "0 auto", display: "flex", flexWrap: "wrap", gap: "2rem", alignItems: "flex-start" }}>
      ...
      <div style={{ maxWidth: "1280px", margin: "0 auto", padding: "clamp(1.5rem,5vw,3rem) clamp(1rem,4vw,2.5rem)" }}>
        {/* VEHICLES */}
              ...
              {publicVehicles.map((v: any) => {
                ...
                return (
                  <div key={v.id} style={{ border: `1px solid ${C.tan}`, background: "white" }}>
                    {/* Vehicle header */}
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(min(100%,280px),1fr))", background: C.dark }}>
        ...
        {(sellerListings?.length ?? 0) > 0 && (
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(160px,1fr))", gap: "1px", background: C.tan, borderRadius: "14px", overflow: "hidden" }}>
