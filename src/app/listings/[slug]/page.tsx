...
export default async function ListingPage({ params }: { params: { slug: string } }) {
  ...
  const valUserIds = rawValuations.map((v: any) => v.user_id).filter((v: any, i: number, a: any[]) => a.indexOf(v) === i);
  ...
  const ownerIds = (vehicleOwners ?? []).map((v: any) => v.owner_id).filter((v: any, i: number, a: any[]) => a.indexOf(v) === i);
